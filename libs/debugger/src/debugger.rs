...
pub struct Debugger<'a> {
    ...
    pub module_cache:  HashMap<PathBuf, Vec<u8>>,
}
...
impl<'a> Debugger<'a> {
    ...
    pub fn spawn_proc(argv: &[String], follow_fork: bool) -> Debugger<'a> {
        ...
        let nul = std::fs::File::create("R:\\no").expect("couldnt get handle to tmp file\n").as_raw_handle();
        //print!("nul: {:p}\n", nul as *mut winapi::ctypes::c_void);

        let mut startup_info: STARTUPINFOA = unsafe { std::mem::zeroed() };
        (startup_info).dwFlags |= winapi::um::winbase::STARTF_USESTDHANDLES;
        (startup_info).hStdError = nul as *mut winapi::ctypes::c_void;
        (startup_info).hStdOutput = nul as *mut winapi::ctypes::c_void;
        (startup_info).hStdInput = nul as *mut winapi::ctypes::c_void;
    ...
    pub fn register_modload_callback(&mut self, func: ModloadFunc, mesofiles: HashMap<PathBuf, Vec<u8>>) {
        self.module_load_callbacks.as_mut()
            .expect("Cannot add callback during callback").push(func);
        
        self.module_cache = mesofiles;
    }
                ...
                EXCEPTION_DEBUG_EVENT => {
                    ...
                    if exception.ExceptionRecord.ExceptionCode == 0x80000003 {
                        ...
                        if exception.ExceptionRecord.ExceptionCode ==
                            ...
                            // Exit out
                            return ExitType::Crash(filename);
