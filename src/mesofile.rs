...
pub fn load_meso(dbg: &mut Debugger, meso_path: &Path) {
    ...
    // Read the meso either from cache or from FS
    let meso: Vec<u8> = {

        match dbg.module_cache.get(meso_path){
            Some(cached_module) => { cached_module.to_vec() }
            None => {  std::fs::read(meso_path).expect("Failed to read meso")  }
        }
    };
